...
ns_format_fun2 = function(data, col = NULL, n = NULL){
  if(is.null(col)) {
    
    
    col = c("Accession", "Collection_Date", "Country",
            "Latitude", "Longitude", "Continent",
            "lineage_merge", "host_class", "host_genus")       #selecting only columns that will go in the build
  }
  ...
  test_final = test %>%
    select(strain, accession, date, country, state, region, host, lineage, host_genus)
